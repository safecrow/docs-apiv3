...
Для преавторизации платежа за сделку в запросе указывается номер `(id)` сделки, которая будет оплачена Покупателем `(consumer)`
```
...
При преавторизации платежа, на карте покупателя блокируется необходимая для оплаты сделки сумма. Для оплаты сделки и списания денег с карты покупателя, преавторизованый платеж необходимо подтвердить.
...
При подверджении преавторизации происходит списание денег с карту покупателя и сделка переходит в статус `paid`.

Для отмены преавторизации и разблокироваки средств на карте покупателя, выполните запрос тыдым
